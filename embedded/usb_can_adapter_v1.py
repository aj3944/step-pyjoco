...
def parse(frame):
    ...
    try:
        if hex(frame[5]) == hex(0):
            ...
            temp = frame[6]
            torque = frame[7]*256 + frame[8]
            speed = frame[9]*256 + frame[10]
            position = frame[11]*256 + frame[12]
            if torque > 2048:
                torque = twos_comp(torque,16)
            if speed > 32_767:
                speed = twos_comp(speed,16)
...
class UsbCanAdapter:
    ...
    def parse_frame(self,print_flag : bool) -> int:
        frame_id = self.data_dict['frame_id']
        motor_id = int(int(frame_id,16) & 0xffff - 0x140) 
        command_id = (int(frame_id,16) >> 16) & 0x00ff
        command_dir = (int(frame_id,16) >> 24) & 0x00ff
        data = self.data_dict['data']
        if print_flag:
            try:
            #     print("<--------------------->")
                # print(f"{self.data_dict}")
                print("command_dir:\t",hex(command_dir))
            ...
            if command_id == 0xa3 and int(command_dir) == 0:
                position =  data[2]*256 + data[3] + data[4]*256*256*256 + data[5]*256*256
                if position > 2**31:
                    position = twos_comp(position,32)
                print("position:\t",position)

            if command_id == 0xa1 and int(command_dir) == 0:
                torque =  data[2]*256 + data[3]
                if torque > 2048:
                    torque = twos_comp(torque,16)
                #     torque = (16**3  - torque)
                print("torque:\t",torque)
            if not int(command_dir) == 0:
                print("motor response")
                temp =  data[0];
                torque_current = data[0]*256;
                motor_speed = data[1] + data[2]*256;
                encoder_pos = data[3] + data[4]*256;
                print("temp:\t",temp)
                print("torque_current:\t",torque_current)
    ...
    def dump_data_frames(self, print_flag: bool) -> int:
        ...
        while self.program_running:
            frame_len = self.frame_receive(200)
